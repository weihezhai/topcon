...
class TextDatasetBuilder:
    ...
    # NEW: Unlabeled dataset loader for evaluation-only scenarios
    def load_unlabeled_dataset_with_ids(self):
        """Load JSON files and create dataset with paper IDs (no labels required)"""
        texts = []
        paper_ids = []
        ...
        # Process subdirectories in sorted order for consistency
        for paper_id in sorted(os.listdir(self.data_folder)):
            subdir_path = os.path.join(self.data_folder, paper_id)
            if not os.path.isdir(subdir_path):
                continue
    ...
    # NEW: simple caching helper
    def cache_dataset(self, dataset, cache_path):
        ...
        try:
            ...
            print(f"Failed to cache dataset to {cache_path}: {e}")