...
class TextDatasetBuilder:
    def __init__(self, data_folder, labels_file, statistics_file=None, metadata_files=None, max_length=1024):
        self.data_folder = data_folder
        self.labels_file = labels_file
        self.statistics_file = statistics_file
        # Support both single file path (str) and list of paths
    ...
    def load_metadata(self):
        """Load metadata from JSON files (list of paper objects)"""
        meta_dict = {}
        ...
        for meta_file in self.metadata_files:
            ...
            try:
                ...
                # Later files overwrite earlier ones if duplicate IDs exist
                for item in data:
                    if 'id' in item:
            ...
            if paper_id in labels_dict:
                ...
                try:
                    ...
                    if text:  # Only add non-empty texts
                        ...
                        # Extract metadata scores (taking the mean from [mean, std] list)
                        s_score = 0.0
                        c_score = 0.0
                        p_score = 0.0
                        
                        if paper_id in meta_dict:
                            m = meta_dict[paper_id]
                            # Helper to extract first element if list, or value if number
                            def get_score(val):
                            ...
                            s_score = get_score(m.get('soundness_avg'))
                            c_score = get_score(m.get('contribution_avg'))
                            p_score = get_score(m.get('presentation_avg'))
                        
                        soundness_scores.append(s_score)
                        contribution_scores.append(c_score)
                        presentation_scores.append(p_score)
