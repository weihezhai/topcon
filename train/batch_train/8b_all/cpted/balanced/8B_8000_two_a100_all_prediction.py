...
def split_dataset_stratified(dataset, test_size=0.2, seed=42):
    ...
    # Preserve original columns (text, labels, paper_id)
    return {
        ...
        with torch.no_grad():
            ...
            if len(batch_input_ids) > 0:
                ...
                for j, (decision_pos, true_label) in enumerate(zip(decision_positions, true_labels)):
                    logits_at_pos = logits[j, decision_pos - 1]
...
def main():
    ...
    try:
        ...
        parser.add_argument("--output_dir", type=str, default="/mnt/parscratch/users/acr24wz/etu/topcon/qwen3_8B/cpt_model/balanced/finetuned/all", help="Directory to save/load the fine-tuned model")
