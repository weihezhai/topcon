...
from hierarchical_collator import HierarchicalDataCollator
from hierarchical_model import HierarchicalPromptModel
...
def preprocess_hierarchical(examples, chunker):
    """Preprocess examples for hierarchical model."""
    processed = []
    for text, label in zip(examples['text'], examples['labels']):
        result = chunker.process_document(text, label)
        processed.append(result)
    
    return {
        'chunks': [p['chunks'] for p in processed],
        'chunk_masks': [p['chunk_masks'] for p in processed],
        'labels': [p['labels'] for p in processed],
        'num_sentences': [p['num_sentences'] for p in processed]
    }
...
def main():
    ...
    try:
        ...
        parser.add_argument("--use_sentence_chunks", action="store_true", 
                           help="Use hierarchical sentence-based model with soft tokens")
        parser.add_argument("--max_sentences", type=int, default=128, 
                           help="Maximum sentences per document")
        parser.add_argument("--max_sentence_length", type=int, default=64,
                           help="Maximum tokens per sentence")
        parser.add_argument("--max_selected_sentences", type=int, default=32,
                           help="Maximum sentences to select (M)")
        parser.add_argument("--k_soft_tokens_per_chunk", type=int, default=4,
                           help="Soft tokens per selected sentence (K)")
        parser.add_argument("--prompt_prefix", type=str, 
                           default="Analyze the following scientific paper:\n",
                           help="Prompt prefix text")
        parser.add_argument("--prompt_suffix", type=str,
                           default="\nBased on the content, should this paper be accepted? Answer yes or no.\nDecision:",
                           help="Prompt suffix text")
        parser.add_argument("--freeze_backbone", action="store_true",
                           help="Freeze backbone in stage 1 training")
