...
# New: renderer returning the same table as a string for saving
...
def parse_args():
    ...
    p.add_argument(
        "--model_path",
        default="/mnt/parscratch/users/acr24wz/etu/topcon/qwen3_8B/cpt_model/imbalanced/finetuned/all/checkpoint-1200",
    ...
    # New: path to save the last printed table (bin table)
...
def main():
    ...
    # New: also save the last printed table (bin table) if requested
    if args.output_bin_table:
        bin_table_text = render_bin_table(bin_rows)
        with open(args.output_bin_table, "w") as f:
            f.write(bin_table_text + "\n")
