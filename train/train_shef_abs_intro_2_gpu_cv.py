...
def download_and_save_model(model_name, cache_dir):
    ...
    # Download model for causal language modeling without loading to GPU
...
def batched_accuracy_evaluation(trainer, eval_dataset, batch_size=10, detailed_eval=False, tokenizer=None, device_ids=None):
    ...
    for i in range(0, len(eval_dataset), batch_size):
        ...
        with torch.no_grad():
            ...
            if len(batch_input_ids) > 0:
                ...
                # Convert to tensors and move to first device (model parallel will handle distribution)
                input_tensor = torch.tensor(padded_input_ids, device=f'cuda:{device_ids[0] if device_ids else 0}')
                mask_tensor = torch.tensor(padded_attention_masks, device=f'cuda:{device_ids[0] if device_ids else 0}')
...
def main():
    ...
    try:
        ...
        parser.add_argument("--gpu_id", type=int, default=0, help="GPU ID to use for training/evaluation (deprecated, use --gpu_ids)")
        parser.add_argument("--gpu_ids", type=str, default=None, help="Comma-separated GPU IDs to use for model parallel (e.g., '0,1')")
        args = parser.parse_args()
        
        # Parse GPU IDs
        if args.gpu_ids:
            gpu_ids = [int(x.strip()) for x in args.gpu_ids.split(',')]
            print(f"Using GPUs: {gpu_ids} for model parallel")
        else:
            # Fallback to single GPU mode
            gpu_ids = [args.gpu_id]
            print(f"Using single GPU ID: {args.gpu_id}")
        
        # Set CUDA_VISIBLE_DEVICES to only use the specified GPUs
        os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(map(str, gpu_ids))
        
        # After setting CUDA_VISIBLE_DEVICES, GPUs will be renumbered starting from 0
        ...
        if torch.cuda.is_available():
            for i, gpu_id in enumerate(device_ids):
                ...
                print(f"GPU {gpu_ids[i]} (device {gpu_id}): {torch.cuda.get_device_name(gpu_id)}")
                gpu_memory = torch.cuda.get_device_properties(gpu_id).total_memory / 1024**3
                print(f"GPU {gpu_ids[i]} memory: {gpu_memory:.1f} GB")
        else:
        ...
        # Load model with model parallelism across specified GPUs
        if len(device_ids) > 1:
            print(f"Loading model with model parallelism across {len(device_ids)} GPUs...")
            ...
            if args.eval:
                ...
                model = AutoModelForCausalLM.from_pretrained(
                    ...
                    max_memory={i: f"{int(torch.cuda.get_device_properties(i).total_memory * 0.85 / 1024**3)}GiB" for i in device_ids}
                )
                print("Loaded fine-tuned model for evaluation with model parallelism")
            else:
                ...
                print("Loaded base model for training with model parallelism")
        else:
            # Single GPU mode
            if args.eval:
                model = AutoModelForCausalLM.from_pretrained(
                    OUTPUT_DIR,
                    torch_dtype=torch.bfloat16,
                    device_map={"": f"cuda:{device_ids[0]}"}
                )
                print("Loaded fine-tuned model for evaluation on single GPU")
            else:
                model = AutoModelForCausalLM.from_pretrained(
                    BASE_MODEL_CACHE,
                    torch_dtype=torch.bfloat16,
                ...
                print("Loaded base model for training on single GPU")

        print(model.hf_device_map if hasattr(model, 'hf_device_map') else "No device map available")
        ...
        if not args.eval:
            # Training arguments - adjusted for model parallel
            training_args = TrainingArguments(
                ...
                ddp_find_unused_parameters=False if len(device_ids) > 1 else None,  # For model parallel
            )
            ...
            # Reload the model fresh with model parallelism
            if len(device_ids) > 1:
                model = AutoModelForCausalLM.from_pretrained(
        ...
        training_args_eval = TrainingArguments(
            ...
            ddp_find_unused_parameters=False if len(device_ids) > 1 else None,
        )
