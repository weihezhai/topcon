'''
./models/
├── base_model/          # Original model cache
│   ├── config.json
│   ├── tokenizer.json
│   └── pytorch_model.bin
└── finetuned_model/     # Fine-tuned full model
    ├── config.json
    ├── pytorch_model.bin
    └── tokenizer files
'''
...
def main():
    ...
    # Load model from cached location - full model fine-tuning
    model = AutoModelForSequenceClassification.from_pretrained(
        BASE_MODEL_CACHE,
        num_labels=2,
        torch_dtype=torch.float16
    ...
    # Training arguments - adjusted for full model fine-tuning
