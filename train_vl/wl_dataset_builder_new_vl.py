...
class TextDatasetBuilder:
    ...
    def load_metadata_ratings(self):
        """Load metadata and create a mapping of paper_id -> rating_avg"""
        ...
        try:
            ...
            # Create mapping from id to rating_avg
            rating_map = {}
            for item in metadata_list:
                if 'id' in item and 'rating_avg' in item:
                    rating_map[item['id']] = item['rating_avg']
            return rating_map
        except Exception as e:
            print(f"Error loading metadata: {e}")
            return {}

    def calculate_sample_weight(self, rating_avg):
        """
        Calculate sample weight based on rating_avg.
        Down-weight noisy examples (5.4 <= rating <= 6.2) to 0.5.
    ...
    def load_dataset_with_ids(self):
        ...
        # Load metadata ratings
        rating_map = self.load_metadata_ratings()
            ...
            if paper_id in labels_dict:
                ...
                try:
                    ...
                    if text:  # Only add non-empty texts
                        ...
                        # Calculate weight
                        rating = rating_map.get(paper_id)
                        weight = self.calculate_sample_weight(rating)
