...
def _build_deepspeed_config(stage: int = 3):
    """
    Minimal DeepSpeed ZeRO config for HF Trainer.
    Uses "auto" so HF TrainingArguments stays the single source of truth.
    """
    stage = int(stage)
    if stage not in (2, 3):
        raise ValueError("deepspeed_stage must be 2 or 3")
    ...
    if stage == 3:
        cfg["zero_optimization"].update(
            {
                # Crucial so `trainer.save_model()` / `save_pretrained()` can materialize weights when saving
                "stage3_gather_16bit_weights_on_model_save": True,
                # Helps avoid full materialization during init on big models
...
def main():
    # --- distributed awareness (torchrun) ---
    ...
    # Create log filename with timestamp (+ rank to avoid multi-proc clobber)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = os.path.join(log_dir, f"training_log_all_{timestamp}.rank{rank}.log")

    # Redirect stdout/stderr (per-rank file avoids corruption)
    ...
    print(f"Distributed: {is_distributed} (rank={rank}, local_rank={local_rank}, world_size={world_size})")
    print("="*80)
    ...
    try:
        ...
        # +++ DeepSpeed (recommended vs single-process device_map model-parallel) +++
        parser.add_argument(
            "--use_deepspeed",
            action="store_true",
            help="Use DeepSpeed ZeRO (multi-process sharded training). Requires torchrun.",
        )
        parser.add_argument(
            "--deepspeed_stage",
            type=int,
            default=3,
            choices=[2, 3],
            help="DeepSpeed ZeRO stage (3 shards params+grads+optim; 2 shards grads+optim).",
        )
        ...
        # IMPORTANT: in distributed mode, all ranks MUST share the exact same OUTPUT_DIR.
        if not args.eval:
            if args.resume_from_checkpoint:
                ...
                if is_distributed:
                    ...
                    if rank == 0:
                        print(
                            "NOTE: Distributed run detected; not appending timestamp to output_dir. "
                            "Pass a unique --output_dir per run if needed."
                        )
                else:
        ...
        # NOTE:
        # - For TRAINING with DeepSpeed: DO NOT use device_map="auto" (single-process model-parallel).
        #   Load on CPU; DeepSpeed/Trainer will shard and place.
        # - For EVAL: device_map="auto" is fine for single-process inference.
        use_ds = (not args.eval) and args.use_deepspeed

        if use_ds and not is_distributed:
            raise RuntimeError("`--use_deepspeed` requires torchrun (multi-process).")

        if args.eval or (not use_ds):
            # --- existing single-process path (kept as fallback / eval) ---
            base_model = AutoModelForCausalLM.from_pretrained(
                BASE_MODEL_CACHE,
                torch_dtype=torch.bfloat16,
                device_map="auto",
                max_memory={i: "39GiB" for i in range(torch.cuda.device_count())},
                offload_folder="./offload",
                attn_implementation="sdpa",
            )
        else:
            # --- DeepSpeed training path (efficient multi-GPU sharding) ---
            base_model = AutoModelForCausalLM.from_pretrained(
                BASE_MODEL_CACHE,
                torch_dtype=torch.bfloat16,
                device_map=None,              # critical: let DeepSpeed handle placement/sharding
        ...
        if not args.eval:
            ...
            ds_cfg = _build_deepspeed_config(args.deepspeed_stage) if use_ds else None
            ...
            training_args = TrainingArguments(
                ...
                deepspeed=ds_cfg,  # <-- enables ZeRO sharding when --use_deepspeed
            )
            ...
            try:
                ...
                save_emergency_checkpoint(
                    trainer=trainer,
                    model=model if (not is_distributed or rank == 0) else None,
                    tokenizer=tokenizer if (not is_distributed or rank == 0) else None,
                    output_dir=OUTPUT_DIR,
                    reason=reason,
                    exc=e,
                )
            ...
            # Save adapters/tokenizer once (rank0 only)
            if (not is_distributed) or trainer.is_world_process_zero():
